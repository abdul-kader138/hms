...
//  a.kader
    public function add_pDocument()
    {
        $this->form_validation->set_rules('document_name', $this->lang->line('document') . " " . $this->lang->line('name'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            $msg = array(
            ...
            $data = array(
                ...
                'patient_id' => $this->input->post("patient"),
                'document_date' => date('Y-m-d', $this->customlib->datetostrtotime($document_date)),
                'note' => $this->input->post("note"),
            );
            if (isset($_FILES["doc_path"]) && !empty($_FILES['doc_path']['name'])) {
                $insert_id = $this->patient_model->add_pDocument($data);
                if ($insert_id) {
                    $fileInfo = pathinfo($_FILES["doc_path"]["name"]);
                    $img_name = $insert_id . '.' . $fileInfo['extension'];
                    move_uploaded_file($_FILES["doc_path"]["tmp_name"], "./uploads/patient_document/" . $img_name);
                    $data_img = array('id' => $insert_id, 'doc_path' => 'uploads/patient_document/' . $img_name);
                    $this->patient_model->add_pDocument($data_img);
                    $array = array('status' => 'success', 'error' => '', 'message' => 'Record Added Successfully.');
                } else  $array = array('status' => 'fail', 'error' => 'Data insertion failed.Please contact with admin.', 'message' => '');
            } else  $array = array('status' => 'fail', 'error' => 'Please select the file.', 'message' => '');
    ...
    public function deleteIpdPatientDocuments($pateint_id, $id)
    {
        ...
        $patient_document = $this->patient_model->getDocumentDetailsById($id);
        unlink($patient_document->doc_path);
        $this->patient_model->deleteIpdPatientDocuments($id);
        $this->session->set_flashdata('msg', '<div class="alert alert-success">Patient document deleted successfully</div>');
...
//  a.kader
    ...
    public function ipdReport()
    {
        ...
        $additional_where = array("patients.is_active = 'yes'");
        $doctorid = $this->input->post('doctor');

        if (!empty($doctorid)) {
            $additional_where = array("patients.is_active = 'yes' ", "ipd_details.cons_doctor ='" . $doctorid . "'");
        }
        $disable_option = FALSE;
        $userdata = $this->customlib->getUserData();
        $role_id = $userdata['role_id'];
    ...
    public function dischargepatientReport()
    {
        ...
        $table_name = "ipd_details";
        $additional_where = array("patients.is_active = 'no'");
        $doctorid = $this->input->post('doctor');
        ...
        if (!empty($doctorid)) {
            $additional_where = array("patients.is_active = 'no' ", "ipd_details.cons_doctor ='" . $doctorid . "'");
        }
        $search_type = $this->input->post("search_type");
        if (isset($search_type)) {
            $search_type = $this->input->post("search_type");
        ...
        if (!empty($resultlist)) {
            foreach ($data['resultlist'] as $key => $value) {
                $charges = $this->patient_model->getCharges($value["pid"]);
                $data['resultlist'][$i]["charges"] = $charges['charge'];
                $discharge_details = $this->patient_model->getIpdBillDetails($value["pid"]);
                $payment = $this->patient_model->getPayment($value["pid"]);
                $data['resultlist'][$i]["discharge_date"] = $discharge_details['date'];
                $data['resultlist'][$i]["other_charge"] = $discharge_details['other_charge'];
                $data['resultlist'][$i]["tax"] = $discharge_details['tax'];
                $data['resultlist'][$i]["discount"] = $discharge_details['discount'];
