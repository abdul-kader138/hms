...
//    a.kader
    ...
    function add_pReminder($data)
    {
        if (isset($data["id"])) {
            $this->db->where("id", $data["id"])->update("patient_reminder", $data);
        } else {
            $this->db->insert("patient_reminder", $data);
            return $this->db->insert_id();
        }
    }
    ...
    {
        $query = $this->db->where('patient_id', $id)->get('patient_reminder');
        if ($query->num_rows() > 0) {
            foreach (($query->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function deleteIpdPatientReminder($id)
    {
        $this->db->where('id', $id)
