...
Class Bed_Model extends CI_Model {
    ...
    public function bed_list($id = null) {
        $data  = array();
        $this->db->select('bed.*, bed_type.name as bed_type_name,floor.name as floor_name, bed_group.name as bedgroup,bed_group.id as bedgroupid,patients.id as pid,patients.is_active as patient_status,patients.patient_unique_id,patients.patient_name,patients.gender,patients.guardian_name,patients.mobileno,ipd_details.date,staff.name as staff,staff.surname')->from('bed');
